    ...
    return (
        <DashboardLayout>
            <div className="flex h-[calc(100vh-8rem)] gap-6">
                ...
                <div className="w-64 bg-white dark:bg-gray-800 shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 dark:border-gray-700/50 rounded-2xl hidden md:flex flex-col overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-6 text-sm">
                        ...
                        <div>
                            <p className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-3 px-2">Your Conversations</p>
                            <div className="space-y-1">
                                {chatHistory.length === 0 && <p className="px-2 text-xs text-gray-400 dark:text-gray-500 font-medium italic">No recent chats.</p>}
                                {chatHistory.map(chat => (
                                    <button
                                        key={chat.id}
                                        onClick={() => loadChat(chat.id)}
                                        className={`w-full text-left truncate px-3 py-2.5 rounded-lg font-medium flex items-center gap-2 transition-colors ${currentChatId === chat.id ? 'bg-[#4f46e5]/10 text-[#4f46e5]' : 'hover:bg-gray-50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300'}`}
                                    >
                                        <MessageSquare size={16} className={currentChatId === chat.id ? "text-[#4f46e5] flex-shrink-0" : "text-gray-400 dark:text-gray-500 flex-shrink-0"} />
                ...
                {/* Main Chat Interface */}
                <div className="flex-1 bg-white dark:bg-gray-800 rounded-2xl flex flex-col overflow-hidden shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 dark:border-gray-700/50">

                    {/* Header */}
                    <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700/50 flex items-center gap-4 bg-gray-50 dark:bg-gray-700/50/50">
                        <div className="w-12 h-12 rounded-xl bg-[#4f46e5]/10 flex items-center justify-center text-[#4f46e5] shadow-sm">
                            <Bot size={24} />
                        </div>
                        <div>
                            <h2 className="font-bold text-lg text-gray-900 dark:text-white">AceCoach AI</h2>
                    ...
                    <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50 dark:bg-gray-700/50/30">
                        {messages.map((m) => (
                            <div key={m.id} className={`flex gap-4 ${m.type === 'user' ? 'flex-row-reverse' : ''}`}>
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${m.type === 'user' ? 'bg-gradient-to-br from-[#4f46e5] to-[#7c3aed] text-white' : 'bg-white dark:bg-gray-800 shadow-[0_4px_15px_rgb(0,0,0,0.05)] border border-gray-100 dark:border-gray-700/50 text-[#4f46e5]'}`}>
                                    {m.type === 'user' ? <UserIcon size={18} /> : <Bot size={18} />}
                                </div>
                                <div className={`max-w-[75%] rounded-2xl p-4 shadow-[0_4px_20px_rgb(0,0,0,0.04)] text-[15px] leading-relaxed font-medium whitespace-pre-wrap ${m.type === 'user' ? 'bg-gradient-to-br from-[#4f46e5] to-[#7c3aed] text-white rounded-tr-sm border border-indigo-500/20' : 'bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700/50 text-gray-800 rounded-tl-sm'}`}>
                                    ...
                                    {m.attachedFile && (
                                        <div className="mb-3">
                                            {m.attachedFile.preview ? (
                                                ...
                                                <div className="px-3 py-2 bg-white dark:bg-gray-800/20 rounded-lg flex items-center gap-2 max-w-max border border-white/30 truncate">
                                                    <FileText size={16} />
                        ...
                        {isTyping && (
                            <div className="flex gap-4">
                                <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm bg-gray-100 text-[#4f46e5]">
                                    <Bot size={18} />
                                </div>
                                <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700/50 rounded-2xl rounded-tl-sm px-5 py-4 flex items-center space-x-1shadow-sm">
                    ...
                    <div className="p-4 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700/50">
                        ...
                        {attachedFile && (
                            <div className="mb-3">
                                <div className="inline-flex items-center gap-2 px-3 py-2 bg-gray-100 rounded-lg border border-gray-200 dark:border-gray-700">
                                    {attachedFile.type === 'image' ? <ImageIcon size={16} className="text-gray-500 dark:text-gray-400 dark:text-gray-500" /> : <FileText size={16} className="text-gray-500 dark:text-gray-400 dark:text-gray-500" />}
                                    <span className="text-xs font-semibold text-gray-700 dark:text-gray-300 max-w-[150px] truncate">{attachedFile.name}</span>
                                    <button type="button" onClick={() => setAttachedFile(null)} className="p-1 hover:bg-gray-200 rounded-full text-gray-500 dark:text-gray-400 dark:text-gray-500 transition-colors">
                                        <X size={14} />
                                    </button>
                                </div>
                            </div>
                        ...
                        <form onSubmit={handleSend} className="relative flex items-center">
                            <button
                                ...
                                className="absolute left-4 text-gray-400 dark:text-gray-500 hover:text-[#4f46e5] transition-colors cursor-pointer p-1 rounded-md hover:bg-gray-100"
                            >
                                <Paperclip size={20} />
                            </button>
                            ...
                            <input
                                ...
                                className="w-full bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-xl py-4 pl-14 pr-28 outline-none focus:ring-2 focus:ring-[#4f46e5]/40 focus:border-[#4f46e5] transition-all font-medium text-sm placeholder:text-gray-400 dark:text-gray-500"
                            />
                            ...
                            <div className="absolute right-2 flex items-center gap-1">
                                {isRecording ? (
                                    <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-50 text-red-500 animate-pulse border border-red-100">
                                        ...
                                        <button type="button" onClick={() => setIsRecording(false)} className="ml-1 text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-red-700">
                                            <X size={14} />
                                        </button>
                                    </div>
                                ) : (
                                    <button type="button" onClick={() => setIsRecording(true)} className="p-2 text-gray-400 dark:text-gray-500 hover:text-[#4f46e5] rounded-lg hover:bg-gray-100 transition-colors">
                                        <Mic size={20} />
                                    </button>
                                )}
                                <button
