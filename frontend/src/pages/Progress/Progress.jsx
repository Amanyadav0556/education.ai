...
const KPICard = ({ title, value, icon: Icon, colorClass, increase, onClick }) => (
 <div onClick={onClick} className="bg-bg-surface p-5 rounded-3xl border border-border-base shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-md hover:border-purple-200 cursor-pointer active:scale-95 transition-all group">
 ...
 {increase && <span className="text-xs font-bold text-emerald-500 flex items-center bg-emerald-500/10 px-2 py-1 rounded-md"><ArrowUpRight size={12} /> {increase}</span>}
 </div>
 <p className="text-[11px] font-black text-text-muted uppercase tracking-widest">{title}</p>
 <h3 className="text-2xl font-black text-text-main mt-1">{value}</h3>
 </div>
);

const SubjectCard = ({ title, progress, solved, total, accuracy, lastPracticed, icon: Icon, onClick }) => (
 <div onClick={onClick} className="bg-bg-surface p-5 rounded-3xl border border-border-base shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgba(79,70,229,0.08)] hover:border-purple-100 transition-all cursor-pointer group active:scale-95">
 <div className="flex items-center gap-3 mb-4">
 <div className="w-10 h-10 rounded-2xl bg-purple-500/10 text-[#9884d1] flex items-center justify-center group-hover:bg-[#9884d1] group-hover:text-white transition-colors">
 <Icon size={18} />
 </div>
 <div>
 <h4 className="font-extrabold text-text-main text-sm group-hover:text-[#9884d1] transition-colors">{title}</h4>
 <p className="text-[10px] font-black text-text-muted uppercase tracking-widest">Last: {lastPracticed}</p>
 </div>
 </div>
 ...
 <div className="w-full bg-bg-surface-hover h-2 rounded-full mb-4 overflow-hidden relative">
 <div className="bg-[#9884d1] h-full rounded-full transition-all duration-1000" style={{ width: `${progress}%` }}></div>
 </div>
 <div className="grid grid-cols-2 gap-3 mb-4">
 <div className="bg-bg-surface-hover p-2.5 rounded-xl border border-border-base">
 <p className="text-[9px] font-black uppercase text-text-muted">Solved</p>
 <p className="text-sm font-black text-text-main">{solved}<span className="text-xs text-text-muted">/{total}</span></p>
 </div>
 <div className="bg-bg-surface-hover p-2.5 rounded-xl border border-border-base">
 <p className="text-[9px] font-black uppercase text-text-muted">Accuracy</p>
 <p className="text-sm font-black text-emerald-600">{accuracy}%</p>
 </div>
 </div>
 <button className="w-full py-2.5 bg-bg-surface-hover group-hover:bg-purple-500/10 text-text-sub group-hover:text-[#9884d1] font-bold text-xs rounded-xl transition-colors border border-transparent group-hover:border-purple-100 select-none">
 Continue Learning
 </button>
 </div>
);
...
const HeatmapCell = ({ intensity }) => {
 const colors = ['bg-bg-surface-hover', 'bg-purple-200', 'bg-purple-400', 'bg-purple-600', 'bg-purple-800'];
...
export default function Progress() {
 ...
 <div className="bg-bg-surface rounded-[32px] p-8 max-w-lg w-full shadow-2xl relative animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
 <button onClick={() => setSelectedKpi(null)} className="absolute top-6 right-6 text-text-muted hover:text-text-main p-2 rounded-full hover:bg-bg-surface-hover transition-colors">
 ...
 <div>
 <h2 className="text-2xl font-extrabold text-text-main">{selectedKpi.title} Insights</h2>
 <p className="text-sm font-bold text-text-sub mt-0.5">Current Standing: <span className={selectedKpi.colorClass.replace('bg-', 'text-').replace('-500', '-600')}>{selectedKpi.value}</span></p>
 </div>
 </div>

 <div className="space-y-4 mb-8">
 <div className="bg-bg-surface-hover p-4 rounded-2xl border border-border-base flex items-center justify-between">
 <div>
 <p className="text-[10px] font-black text-text-muted uppercase tracking-widest mb-1">Recent Trend</p>
 ...
 <div className="bg-bg-surface p-5 rounded-2xl border border-border-base shadow-sm">
 <p className="text-[10px] font-black text-text-muted uppercase tracking-widest mb-4 flex items-center gap-1.5"><Activity size={12} /> 7-Day Performance Graph</p>
 ...
 <div className="absolute top-0 right-0 w-96 h-96 bg-bg-surface rounded-full blur-[80px] -translate-y-1/2 translate-x-1/3"></div>
 ...
 <span className="inline-block px-3 py-1 bg-bg-surface rounded-lg text-[10.5px] font-black uppercase tracking-widest mb-4 border border-white/20 backdrop-blur-md">Learning Analytics</span>
 ...
 <button className="flex-1 md:flex-none justify-center bg-bg-surface hover:bg-bg-surface backdrop-blur-sm border border-white/20 text-white px-5 py-3 rounded-xl font-bold text-xs transition-colors flex items-center gap-2">
 <Download size={14} /> Export Report
 </button>
 <button className="flex-1 md:flex-none justify-center bg-bg-surface text-[#9884d1] hover:bg-purple-500/10 px-6 py-3 rounded-xl font-black text-xs shadow-[0_4px_15px_rgba(0,0,0,0.1)] transition-transform active:scale-95 flex items-center gap-2">
 ...
 <div className="bg-bg-surface p-7 rounded-[32px] border border-border-base shadow-[0_4px_20px_rgb(0,0,0,0.03)]">
 <div className="flex justify-between items-center mb-6">
 <h3 className="text-lg font-extrabold text-text-main flex items-center gap-2"><CalendarDays size={20} className="text-[#9884d1]" /> Activity Map</h3>
 ...
 <div className="mt-5 grid grid-cols-3 divide-x divide-gray-100 border-t border-border-base pt-5">
 <div className="text-center"><p className="text-[10px] font-black text-text-muted uppercase mb-1">Days Studied</p><p className="text-lg font-black text-text-main">42</p></div>
 <div className="text-center"><p className="text-[10px] font-black text-text-muted uppercase mb-1">Missed Days</p><p className="text-lg font-black text-text-main">14</p></div>
 <div className="text-center"><p className="text-[10px] font-black text-text-muted uppercase mb-1">Intensity</p><p className="text-lg font-black text-[#9884d1]">High</p></div>
 </div>
 </div>
 ...
 <h3 className="text-lg font-extrabold text-text-main flex items-center gap-2"><BookOpen size={20} className="text-[#9884d1]" /> Subject Progress</h3>
 ...
 {/* 6. Skill Progress */}
 <div className="bg-bg-surface p-7 rounded-[32px] border border-border-base shadow-[0_4px_20px_rgb(0,0,0,0.03)]">
 <h3 className="text-lg font-extrabold text-text-main mb-6 flex items-center gap-2"><Target size={20} className="text-[#9884d1]" /> Skill Mastery</h3>
 ...
 <span className="font-bold text-sm text-text-sub group-hover:text-[#9884d1] transition-colors">{skill.name}</span>
 <span className="font-black text-sm text-text-main">{skill.val}%</span>
 </div>
 ...
 <div className="bg-bg-surface p-7 rounded-[32px] border border-border-base shadow-[0_4px_20px_rgb(0,0,0,0.03)] pb-8">
 <div className="flex justify-between items-center mb-8">
 <h3 className="text-lg font-extrabold text-text-main flex items-center gap-2"><TrendingUp size={20} className="text-[#9884d1]" /> Accuracy Trend</h3>
 <div className="flex gap-2">
 <span className="text-[10px] font-black uppercase bg-purple-500/10 text-[#9884d1] px-2 py-1 rounded-md">This Month</span>
 <span className="text-[10px] font-black uppercase bg-bg-surface-hover text-text-sub px-2 py-1 rounded-md">Last Month</span>
 </div>
 </div>
 <div className="relative h-48 w-full border-b border-l border-border-base">
 ...
 {/* Grid lines labels */}
 <div className="absolute -left-6 bottom-0 text-[10px] text-text-muted font-bold">0%</div>
 <div className="absolute -left-8 top-0 text-[10px] text-text-muted font-bold">100%</div>
 <div className="absolute left-0 -bottom-6 text-[10px] text-text-muted font-bold">W1</div>
 <div className="absolute right-0 -bottom-6 text-[10px] text-text-muted font-bold">W4</div>
 </div>
 </div>
 ...
 <h3 className="font-extrabold text-text-main mb-6 flex items-center gap-2 relative z-10"><PieChart size={18} className="text-[#9884d1]" /> Learning Overview</h3>
 ...
 <span className="text-3xl font-black text-text-main">42<span className="text-sm">%</span></span>
 </div>
 </div>
 </div>
 <div className="space-y-3 relative z-10">
 <div className="flex justify-between items-center text-sm font-bold bg-bg-surface p-3 rounded-xl border border-border-base shadow-sm">
 <span className="text-text-sub">Weekly Impr.</span>
 <span className="text-emerald-500">+8.5%</span>
 </div>
 <div className="flex justify-between items-center text-sm font-bold bg-bg-surface p-3 rounded-xl border border-border-base shadow-sm">
 <span className="text-text-sub">Topics Mastered</span>
 <span className="text-text-main">14 / 85</span>
 </div>
 <div className="flex justify-between items-center text-sm font-bold bg-bg-surface p-3 rounded-xl border border-border-base shadow-sm">
 <span className="text-text-sub">Goal ETA</span>
 ...
 <button className="w-full bg-bg-surface hover:bg-bg-surface-hover !text-[#9884d1] py-3 rounded-xl font-black text-xs shadow-sm transition-transform active:scale-95 text-center">
 ...
 <div className="bg-bg-surface p-6 rounded-[32px] border border-border-base shadow-sm hover:shadow-md transition-shadow">
 <h3 className="font-extrabold text-text-main mb-5 flex items-center gap-2"><Map size={18} className="text-[#9884d1]" /> Recommended Next</h3>
 ...
 <div key={i} className="flex gap-3 p-3 border border-border-base bg-bg-surface-hover rounded-2xl hover:border-purple-200 hover:bg-purple-500/10/50 transition-colors cursor-pointer group">
 <div className="w-8 h-8 rounded-full bg-bg-surface border border-border-strong text-text-muted flex items-center justify-center group-hover:text-[#9884d1] group-hover:border-[#9884d1]"><rec.icon size={14} /></div>
 <div>
 <p className="text-xs font-extrabold text-text-main">{rec.t}</p>
 <p className="text-[10px] font-bold text-text-sub">{rec.desc}</p>
 </div>
 </div>
 ))}
 </div>
 ...
 {/* 9. Goals Tracker */}
 <div className="bg-bg-surface p-6 rounded-[32px] border border-border-base shadow-sm">
 <div className="flex justify-between items-center mb-5">
 <h3 className="font-extrabold text-text-main flex items-center gap-2"><Target size={18} className="text-emerald-500" /> Active Goals</h3>
 <button className="text-text-muted hover:text-[#9884d1] bg-bg-surface-hover hover:bg-purple-500/10 p-1.5 rounded-lg transition-colors"><Plus size={16} /></button>
 </div>
 <div className="space-y-4">
 <div>
 <div className="flex justify-between text-xs font-bold mb-1.5"><span className="text-text-sub">Complete Arrays</span><span className="text-emerald-600">82%</span></div>
 <div className="w-full h-1.5 bg-bg-surface-hover rounded-full"><div className="w-[82%] h-full bg-emerald-500 rounded-full"></div></div>
 </div>
 <div>
 <div className="flex justify-between text-xs font-bold mb-1.5"><span className="text-text-sub">Solve 300 Problems</span><span className="text-[#9884d1]">245 / 300</span></div>
 <div className="w-full h-1.5 bg-bg-surface-hover rounded-full"><div className="w-[80%] h-full bg-[#9884d1] rounded-full"></div></div>
 </div>
 <div>
 <div className="flex justify-between text-xs font-bold mb-1.5"><span className="text-text-sub">Maintain 30-Day Streak</span><span className="text-yellow-500">18 / 30</span></div>
 <div className="w-full h-1.5 bg-bg-surface-hover rounded-full"><div className="w-[60%] h-full bg-yellow-400 rounded-full"></div></div>
 </div>
 </div>
 </div>
 ...
 <div className="bg-bg-surface p-6 rounded-[32px] border border-border-base shadow-sm">
 <h3 className="font-extrabold text-text-main mb-5 flex items-center gap-2"><Award size={18} className="text-yellow-500" /> Recent Milestones</h3>
 <div className="relative border-l-2 border-border-base ml-3 space-y-5 pb-2">
 ...
 <div className="ml-4">
 <p className="text-xs font-extrabold text-text-main">100 Problems Solved</p>
 <p className="text-[10px] font-bold text-text-muted">2 hours ago</p>
 </div>
 </div>
 <div className="relative">
 ...
 <div className="ml-4">
 <p className="text-xs font-extrabold text-text-main">14-Day Study Streak</p>
 <p className="text-[10px] font-bold text-text-muted">Yesterday</p>
 </div>
 </div>
 <div className="relative">
 <span className="absolute -left-[23px] w-5 h-5 rounded-full bg-bg-surface-hover border-2 border-white flex items-center justify-center text-text-sub"><CheckCircle2 size={10} /></span>
 <div className="ml-4">
 <p className="text-xs font-bold text-text-sub">Finished Arrays Subject</p>
 <p className="text-[10px] font-bold text-text-muted">3 days ago</p>
 </div>
 </div>
 </div>
 <button className="w-full mt-4 bg-bg-surface-hover hover:bg-bg-surface-hover text-text-sub font-bold text-xs py-2 rounded-xl transition-colors border border-border-strong">View All History</button>
 </div>
